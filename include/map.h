...
struct MAP_API DateVersion {
  ...
  int minor;    // 次版本号
  int revision; // 修正版本号
  bool operator==(const DateVersion &op);
};
...
public: // TODO:
  ...
  // ErrorCode plan_route_shortest(const XYZ& start_point, const
  // std::vector<XYZ> &way_points, const XYZ& end_point, Route& route) const;
  ErrorCode plan_route(const XYZWithHDG &start_point_with_hdg,
  ...
  //
  ErrorCode
  calc_lane_center_line_curv(const LaneId &id, const RoadS &s1, const RoadS &s2,
                             double sampling_spacing,
                             std::vector<TracePoint> &centerline) const;
  ...
  ErrorCode
  get_all_parking_space(std::vector<ParkingSpace> &parking_spaces) const;
  bool
  get_parking_space_by_road(const RoadId road_id,
   ...
   */
  ErrorCode
  query_traffic_lights_in_road(const RoadId &road_id,
                               std::vector<ObjectId> &traffic_lights_ids) const;
